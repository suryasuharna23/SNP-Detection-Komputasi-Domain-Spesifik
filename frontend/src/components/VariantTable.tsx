...
export function VariantTable({ variants }: Props) {
  ...
  const Th = ({ children, k }: { children: React.ReactNode; k: SortKey }) => (
    <th
      className="px-3 py-2.5 text-left text-xs font-semibold text-slate-400
                 uppercase tracking-wider cursor-pointer hover:text-slate-200
  ...
  return (
    <div>
      ...
      <div className="flex flex-wrap gap-2 mb-3 items-center">
        ...
        <span className="text-xs text-slate-500 ml-1">{sorted.length} varian</span>
      ...
      {/* Table */}
      <div className="overflow-auto max-h-80 rounded-lg border border-slate-800">
        <table className="w-full text-xs border-collapse">
          ...
          <tbody>
            {sorted.map((v, i) => (
              ...
              >
                <td className="px-3 py-2 font-mono text-slate-300">{v.pos}</td>
                <td className="px-3 py-2">
                  <span className="font-mono text-slate-400">{v.variant_type}</span>
                </td>
                <td className="px-3 py-2 font-mono text-red-400 font-bold">{v.ref}</td>
                <td className="px-3 py-2 font-mono text-blue-400 font-bold">{v.alt}</td>
                ...
                <td className="px-3 py-2 font-mono">
                  {v.ref_codon ? (
                    <>
                      <span className="text-red-400">{v.ref_codon}</span>
                      <span className="text-slate-600"> → </span>
                      <span className="text-blue-400">{v.alt_codon}</span>
                    </>
                  ) : "—"}
                </td>
                <td className="px-3 py-2">
                  {v.ref_aa ? (
                    <>
                      <span className="text-red-400">{v.ref_aa}</span>
                      <span className="text-slate-600">·</span>
                      <span className="text-slate-500 text-[10px]">{v.ref_aa_name}</span>
                      <span className="text-slate-600"> → </span>
                      <span className="text-blue-400">{v.alt_aa}</span>
                      <span className="text-slate-600">·</span>
                      <span className="text-slate-500 text-[10px]">{v.alt_aa_name}</span>
                    </>
                  ) : "—"}
                </td>
                <td className="px-3 py-2">
